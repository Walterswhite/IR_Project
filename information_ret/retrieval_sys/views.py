...
def upload_image(request):
    ...
    if request.method == 'POST':
        ...
        path = os.path.join(r"C:\Users\abhis\Desktop\IR\information_retrieval\Data","media")
        data = img_search.search_images(os.path.join(path , file))
        list = []
        for x in data:
            name = str(x[1])
            list.append("Data/Images/" + name)
            print(list[-1])
